...
static int open_include_file PROTO((char *, struct file_name_list *, U_CHAR *,
                                    struct include_file **));
static char *remap_include_file PROTO((char *, struct file_name_list *));
static int lookup_ino_include PROTO((struct include_file *));

static void finclude PROTO((int, struct include_file *, FILE_BUF *, int,
                            struct file_name_list *));
...
static void pass_thru_directive PROTO((U_CHAR *, U_CHAR *, FILE_BUF *,
                                       struct directive *));
...
static DEFINITION *collect_expansion PROTO((U_CHAR *, U_CHAR *, int,
                                            struct arglist *));
...
static void conditional_skip PROTO((FILE_BUF *, int, enum node_type, U_CHAR *,
                                    FILE_BUF *));
...
static U_CHAR *skip_quoted_string PROTO((U_CHAR *, U_CHAR *, int, int *, int *,
                                         int *));
...
static void output_line_directive PROTO((FILE_BUF *, FILE_BUF *, int,
...
static U_CHAR *macarg1 PROTO((U_CHAR *, U_CHAR *, struct hashnode *, int *,
                              int *, int *, int));
...
static struct file_name_list *
    new_include_prefix PROTO((struct file_name_list *, char *, char *, char *));
static void append_include_chain PROTO((struct file_name_list *,
                                        struct file_name_list *));
...
static int safe_read(desc, ptr, len) int desc;
char *ptr;
int len;
{
...
static void safe_write(desc, ptr, len) int desc;
char *ptr;
int len;
{
  int wcount, written;
...
char **argv;
{
  ...
  /* Get rid of any avoidable limit on stack size.  */
  {
    ...
    /* Set the stack limit huge so that malloc (particularly stringtab
...
#ifdef VMS
  {
    ...
    if ((p = rindex(s, ';')) != 0)
      *p = '\0';                  /* strip version number */
    if ((p = rindex(s, '.')) != 0 /* strip type iff ".exe" */
        && (p[1] == 'e' || p[1] == 'E') && (p[2] == 'x' || p[2] == 'X') &&
        (p[3] == 'e' || p[3] == 'E') && !p[4])
      *p = '\0';
  }
#endif
    ...
    if (argv[i][0] != '-') {
      ...
      case 'i':
        ...
        if (!strcmp(argv[i], "-isystem")) {
          ...
          if (!(dirtmp = new_include_prefix(NULL_PTR, NULL_PTR, "", argv[++i])))
            break;
        ...
        if (!strcmp(argv[i], "-iwithprefix")) {
          ...
          if (!(dirtmp =
                    new_include_prefix(NULL_PTR, NULL_PTR, prefix, argv[++i])))
            break;
      ...
      case 'l':
        ...
        if (!strcmp(argv[i], "-lang-c89"))
          cplusplus = 0, cplusplus_comments = 1, c89 = 1, objc = 0;
        if (!strcmp(argv[i], "-lang-c++"))
  ...
  if (!inhibit_predefs) {
    char *p = (char *)malloc(strlen(predefs) + 1);
    strcpy(p, predefs);
    while (*p) {
      char *q;
      while (*p == ' ' || *p == '\t')
      ...
      if (p[0] == '-' && p[1] == 'D') {
        ...
        past_name = assertion;
        /* Locate end of name.  */
        while (*past_name && *past_name != ' ' && *past_name != '\t' &&
               *past_name != '(')
          past_name++;
        ...
        past_value = value;
        /* Locate end of value.  */
        while (*past_value && *past_value != ' ' && *past_value != '\t' &&
               *past_value != ')')
          past_value++;
    ...
    if (epath) {
      ...
      for (num_dirs = 1, startp = epath; *startp; startp++)
        if (*startp == PATH_SEPARATOR)
          num_dirs++;
      include_defaults = (struct default_include *)xmalloc(
      ...
      bcopy((char *)include_defaults_array, (char *)&include_defaults[num_dirs],
            sizeof(include_defaults_array));
    }
  }
      ...
      for (p = include_defaults; p->fname; p++) {
        ...
        if (!p->cplusplus || (cplusplus && !no_standard_cplusplus_includes)) {
          ...
          if (!strncmp(p->fname, default_prefix, default_len)) {
            ...
            struct file_name_list *new = new_include_prefix(
                NULL_PTR, NULL_PTR, specd_prefix, p->fname + default_len);
            if (new) {
  ...
  if (print_deps) {
    ...
    if (deps_target) {
      ...
      if (len >= 2 && p[len - 2] == '.' && index("cCsSm", p[len - 1]))
        q = p + (len - 2);
      else if (len >= 3 && p[len - 3] == '.' && p[len - 2] == 'c' &&
               p[len - 1] == 'c')
        q = p + (len - 3);
      else if (len >= 4 && p[len - 4] == '.' && p[len - 3] == 'c' &&
               p[len - 2] == 'x' && p[len - 1] == 'x')
        q = p + (len - 4);
      else if (len >= 4 && p[len - 4] == '.' && p[len - 3] == 'c' &&
               p[len - 2] == 'p' && p[len - 1] == 'p')
...
static void path_include(path) char *path;
{
  char *p;
  ...
  if (*p)
    while (1) {
      ...
      q[-1] = 0;
      dirtmp = new_include_prefix(last_include, NULL_PTR, "", p == q ? "." : p);
      q[-1] = c;
      append_include_chain(dirtmp, dirtmp);
...
static U_CHAR *index0(s, c, n) U_CHAR *s;
int c;
size_t n;
{
  char *p = (char *)s;
...
static void trigraph_pcp(buf) FILE_BUF *buf;
{
...
static void newline_fix(bp) U_CHAR *bp;
{
...
static void name_newline_fix(bp) U_CHAR *bp;
{
  register U_CHAR *p = bp;
...
static char *get_lintcmd(ibp, limit, argstart, arglen,
                         cmdlen) register U_CHAR *ibp;
...
static void rescan(op, output_marks) FILE_BUF *op;
int output_marks;
{
 ...
 */
static FILE_BUF expand_to_temp_buffer(buf, limit, output_marks,
                                      assertions) U_CHAR *buf,
    *limit;
...
static int handle_directive(ip, op) FILE_BUF *ip, *op;
{
  ...
  for (kt = directive_table; kt->length > 0; kt++) {
    ...
    endloop1:
      ...
      if (copy_directive) {
        ...
        cp = (U_CHAR *)malloc(bp - buf + 5); /* room for directive plus
                                                some slop */
        buf = cp;
        ...
        while (xp < bp) {
          ...
          *cp++ = c;
          ...
          case '\'':
          case '\"': {
            register U_CHAR *bp1 = skip_quoted_string(
                xp - 1, bp, ip->lineno, NULL_PTR, NULL_PTR, NULL_PTR);
            while (xp != bp1)
              *cp++ = *xp++;
          } break;

          case '/':
            if (*xp == '*' || (cplusplus_comments && *xp == '/')) {
              ip->bufp = xp + 1;
...
static void special_symbol(hp, op) HASHNODE *hp;
FILE_BUF *op;
{
  char *buf;
  int i, len;
  ...
  case T_BASE_FILE: {
    ...
    if (string) {
      buf = (char *)malloc(3 + 4 * strlen(string));
  ...
  case T_INCLUDE_LEVEL:
    ...
    buf = (char *)malloc(8); /* Eight bytes ought to be more than enough */
    sprintf(buf, "%d", true_indepth - 1);
    break;

  case T_VERSION:
    buf = (char *)malloc(3 + strlen(version_string));
  ...
  case T_CONST:
    ...
    if (ip->system_header_p && hp->length == 8 &&
        bcmp(hp->name, "__STDC__", 8) == 0 &&
  ...
  case T_SPEC_DEFINED:
    ...
    if ((hp = lookup(ip->bufp, -1, -1))) {
      ...
      buf = " 1 ";
    } else if (pcp_outfile && pcp_inside_if) {
      /* Output a precondition for this macro use */
      U_CHAR *cp = ip->bufp;
...
static int do_include(buf, limit, op, keyword) U_CHAR *buf, *limit;
FILE_BUF *op;
struct directive *keyword;
{
  ...
  if (importing && warn_import && !inhibit_warnings &&
    ...
    fprintf(stderr, "The fact that a certain header file need not be processed "
                    "more than once\n");
    fprintf(stderr,
...
get_filename:
  ...
  while (limit != fin && is_hor_space[limit[-1]])
    limit--;
  fbeg = fend = (char *)malloc(limit - fin);
  ...
  case '\"': {
    ...
    /* Copy the operand text, concatenating the strings.  */
    {
      for (;;) {
        ...
        while (fin != limit && is_hor_space[*fin])
          fin++;
        if (fin != limit && *fin == '\"')
          fin++;
        else
    ...
    for (fp = &instack[indepth]; fp >= instack; fp--) {
      ...
      if ((nam = fp->nominal_fname) != NULL) {
        ...
        dsp = ((struct file_name_list *)malloc(sizeof(struct file_name_list) +
                                               strlen(nam)));
  ...
  fail:
    if (!retried) {
      ...
      src = trybuf.buf;
      buf = (U_CHAR *)malloc(trybuf.bufp - trybuf.buf + 1);
      limit = buf;
      while (src != trybuf.bufp) {
        ...
        case '\'':
        case '\"': {
          U_CHAR *src1 = skip_quoted_string(src - 1, trybuf.bufp, 0, NULL_PTR,
                                            NULL_PTR, NULL_PTR);
          while (src != src1)
            *limit++ = *src++;
        } break;
  ...
  if (absolute_filename(fbeg)) {
    ...
    for (searchptr = search_start; searchptr; searchptr = searchptr->next) {
      ...
      if (searchptr == first_bracket_include) {
        ...
        struct bypass_dir *p;
        bypass_slot =
            &bypass_hashtab[hashf((U_CHAR *)fbeg, flen, INCLUDE_HASHSIZE)];
  ...
  if (f < 0) {
    ...
    if (f == -2) {
      ...
      if (angle_brackets) {
        if (search_start) {
          char *p =
              (char *)malloc(strlen(search_start->fname) + strlen(fbeg) + 1);
    ...
    pcftry = (char *)malloc(strlen(fname) + 30);
    pcfbuf = 0;
    pcfnum = 0;
...
static int is_system_include(filename) register char *filename;
{
  struct file_name_list *searchptr;

  for (searchptr = first_system_include; searchptr; searchptr = searchptr->next)
...
static char *base_name(fname) char *fname;
{
  char *s = fname;
  char *p;
  ...
  if ((p = rindex(s, ':')))
    s = p + 1; /* Skip device.  */
  if ((p = rindex(s, ']')))
    s = p + 1; /* Skip directory.  */
  if ((p = rindex(s, '>')))
    s = p + 1; /* Skip alternate (int'n'l) dir.  */
  if (s != fname)
    return s;
#endif
  if ((p = rindex(s, '/')))
...
static int absolute_filename(filename) char *filename;
{
...
static size_t simplify_filename(filename) char *filename;
{
    ...
    if (*++from == '/') {
      if (*++from == '/') {
        ...
        if (slashslash_vs_slash == 0) {
          ...
          slashslash_vs_slash =
              ((stat("/", &s1) == 0 && stat("//", &s2) == 0 &&
                INO_T_EQ(s1.st_ino, s2.st_ino) && s1.st_dev == s2.st_dev)
                   ? 1
                   : -1);
        }
        if (slashslash_vs_slash < 0)
...
static char *read_filename_string(ch, f) int ch;
FILE *f;
{
  char *alloc, *set;
  int len;
...
static struct file_name_map *read_name_map(dirname) char *dirname;
{
  ...
  name = (char *)malloc(dirlen + strlen(FILE_NAME_MAP_FILE) + 1);
  strcpy(name, dirname);
  ...
  if (!f)
    ...
    while ((ch = getc(f)) != EOF) {
      ...
      ptr = ((struct file_name_map *)xmalloc(sizeof(struct file_name_map)));
      ptr->map_from = from;
...
static int open_include_file(filename, searchptr, importing,
                             pinc) char *filename;
struct file_name_list *searchptr;
U_CHAR *importing;
struct include_file **pinc;
{
  ...
  struct include_file **phead =
      &include_hashtab[hashf((U_CHAR *)fname, strlen(fname), INCLUDE_HASHSIZE)];
  struct include_file *inc, *head = *phead;
  for (inc = head; inc; inc = inc->next)
    if (!strcmp(fname, inc->fname))
      break;

  if (!inc || !inc->control_macro ||
    ...
    if (!inc) {
      ...
      *phead = inc;
      ...
      if (lookup_ino_include(inc) && inc->control_macro &&
          (!inc->control_macro[0] || lookup(inc->control_macro, -1, -1))) {
        close(fd);
        fd = -2;
      }
...
static char *remap_include_file(filename, searchptr) char *filename;
struct file_name_list *searchptr;
{
  ...
  if (from != filename || !searchptr) {
    ...
    char *dir = (char *)malloc(from - filename + 1);
...
static int lookup_ino_include(inc) struct include_file *inc;
{
...
static void finclude(f, inc, op, system_header_p, dirptr) int f;
struct include_file *inc;
FILE_BUF *op;
int system_header_p;
struct file_name_list *dirptr;
{
  ...
  if (S_ISREG(inc->st.st_mode)) {
    ...
    if (s != (size_t) inc->st.st_size || s + 2 < s)
      memory_full();
    fp->buf = (U_CHAR *)xmalloc(s + 2);
    fp->bufp = fp->buf;
...
static void record_control_macro(inc, macro_name) struct include_file *inc;
U_CHAR *macro_name;
{
...
static char *check_precompiled(pcf, st, fname, limit) int pcf;
struct stat *st;
char *fname;
char **limit;
{
...
static int check_preconditions(prec) char *prec;
{
  MACRODEF mdef;
  char *lineend;
  ...
  while (*prec) {
    ...
    if (!strncmp(prec, "define", 6)) {
      ...
      if ((hp = lookup(mdef.symnam, mdef.symlen, -1)) == NULL ||
          ...
          (hp->type == T_MACRO && !compare_defs(mdef.defn, hp->value.defn) &&
           (mdef.defn->length != 2 || mdef.defn->expansion[0] != '\n' ||
...
static void pcfinclude(buf, limit, name, op) U_CHAR *buf, *limit, *name;
FILE_BUF *op;
{
  FILE_BUF tmpbuf;
  int nstrings;
...
static void pcstring_used(hp) HASHNODE *hp;
{
  KEYDEF *kp;
...
static void write_output() {
  ...
  while (cur_buf_loc < outbuf.bufp || next_string) {
    if (next_string && cur_buf_loc - outbuf.buf == next_string->output_mark) {
      if (next_string->writeflag) {
        len = 4 * strlen((char *)next_string->filename) + 32;
        while (len > line_directive_len)
          line_directive = xrealloc(line_directive, line_directive_len *= 2);
        ...
        safe_write(fileno(stdout), (char *)next_string->contents,
                   next_string->len);
      }
      next_string = next_string->chain;
    } else {
      len =
          (next_string ? (next_string->output_mark - (cur_buf_loc - outbuf.buf))
...
static void pass_thru_directive(buf, limit, op, keyword) U_CHAR *buf, *limit;
FILE_BUF *op;
struct directive *keyword;
{
...
static MACRODEF create_definition(buf, limit, op) U_CHAR *buf, *limit;
FILE_BUF *op;
{
  ...
  if (*bp == '(') {
      ...
      }
      {
        ...
        for (otemp = temp->next; otemp != NULL; otemp = otemp->next)
          if (temp->length == otemp->length &&
            ...
            error("duplicate argument name `%.*s' in `#define'", temp->length,
                  temp->name);
            goto nope;
          }
      }
    ...
    if (bp < limit) {
      if (is_hor_space[*bp]) {
        ...
        case '}':
        case '~':
          warning("missing white space after `#define %.*s'", sym_length,
                  symname);
          break;

        default:
          pedwarn("missing white space after `#define %.*s'", sym_length,
                  symname);
          break;
        }
      }
...
static int do_define(buf, limit, op, keyword) U_CHAR *buf, *limit;
FILE_BUF *op;
struct directive *keyword;
{
  int hashcode;
...

  {
    ...
    if ((hp = lookup(mdef.symnam, mdef.symlen, hashcode)) != NULL) {
      ...
      if (!ok) {
        ...
        if (hp->type == T_MACRO)
          pedwarn_with_file_and_line(
              hp->value.defn->file, hp->value.defn->line,
      ...
      if (debug_output && op)
        pass_thru_directive(buf, limit, op, keyword);
      install(mdef.symnam, mdef.symlen, T_MACRO, (char *)mdef.defn, hashcode);
    }
  }
...
static int check_macro_name(symname, usage) U_CHAR *symname;
char *usage;
{
  U_CHAR *p;
  int sym_length;
...
static int compare_defs(d1, d2) DEFINITION *d1, *d2;
{
  ...
  if (d1->nargs != d2->nargs)
    return 1;
  if (pedantic && strcmp((char *)d1->args.argnames, (char *)d2->args.argnames))
    return 1;
...
static int comp_def_part(first, beg1, len1, beg2, len2, last) int first;
U_CHAR *beg1, *beg2;
int len1, len2;
int last;
{
...
static DEFINITION *collect_expansion(buf, end, nargs, arglist) U_CHAR *buf,
    *end;
int nargs;
struct arglist *arglist;
{
  ...
  maxsize = (sizeof(DEFINITION) + (limit - p) + 3);
        ...
        for (arg = arglist; arg != NULL; arg = arg->next) {
          ...
          if (arg->name[0] == c && arg->length == id_len &&
            ...
            if (expected_delimiter) {
              if (warn_stringify) {
                if (traditional) {
                  warning("macro argument `%.*s' is stringified.", id_len,
                          arg->name);
                } else {
                  warning("macro arg `%.*s' would be stringified with "
                          "-traditional.",
                          id_len, arg->name);
                }
              }
              ...
              tpat_stringify = (stringify == id_beg ? stringify_sharp_token_type
                                                    : NO_SHARP_TOKEN);
            }
...
static int do_assert(buf, limit, op, keyword) U_CHAR *buf, *limit;
FILE_BUF *op;
struct directive *keyword;
{
  U_CHAR *bp;      /* temp ptr into input buffer */
...
static int do_unassert(buf, limit, op, keyword) U_CHAR *buf, *limit;
FILE_BUF *op;
struct directive *keyword;
{
  U_CHAR *bp;      /* temp ptr into input buffer */
...
static int compare_token_lists(l1, l2) struct arglist *l1, *l2;
{
  while (l1 && l2) {
...
static struct arglist *read_token_list(bpp, limit, error_flag) U_CHAR **bpp;
U_CHAR *limit;
int *error_flag;
{
  ...
  *error_flag = 0;
  ...
  while (depth > 0) {
    ...
    if (*bp == '(') {
      ...
      while (!is_hor_space[*bp] && *bp != '(' && *bp != ')' && *bp != '"' &&
             *bp != '\'' && bp != limit)
        bp++;
...
static void free_token_list(tokens) struct arglist *tokens;
{
  while (tokens) {
...
static ASSERTION_HASHNODE *assertion_install(name, len, hash) U_CHAR *name;
int len;
int hash;
{
...
static ASSERTION_HASHNODE *assertion_lookup(name, len, hash) U_CHAR *name;
int len;
int hash;
{
...
static void delete_assertion(hp) ASSERTION_HASHNODE *hp;
{
...
static int do_line(buf, limit, op, keyword) U_CHAR *buf, *limit;
FILE_BUF *op;
struct directive *keyword;
{
  register U_CHAR *bp;
...
static int do_undef(buf, limit, op, keyword) U_CHAR *buf, *limit;
FILE_BUF *op;
struct directive *keyword;
{
  int sym_length;
...
static int do_error(buf, limit, op, keyword) U_CHAR *buf, *limit;
FILE_BUF *op;
struct directive *keyword;
{
  int length = limit - buf;
  U_CHAR *copy = (U_CHAR *)malloc(length + 1);
...
static int do_warning(buf, limit, op, keyword) U_CHAR *buf, *limit;
FILE_BUF *op;
struct directive *keyword;
{
  int length = limit - buf;
...
static int do_ident(buf, limit, op, keyword) U_CHAR *buf, *limit;
FILE_BUF *op;
struct directive *keyword;
{
  FILE_BUF trybuf;
...
static int do_pragma(buf, limit, op, keyword) U_CHAR *buf, *limit;
FILE_BUF *op;
struct directive *keyword;
{
  SKIP_WHITE_SPACE(buf);
    ...
    for (h = 0; h < INCLUDE_HASHSIZE; h++) {
      ...
      for (inc = include_hashtab[h]; inc; inc = inc->next) {
        if (!strcmp(base_name(inc->fname), (char *)fname)) {
          warning(
              "`#pragma implementation' for \"%s\" appears after its #include",
              fname);
          return 0;
        }
      }
...
static int do_sccs(buf, limit, op, keyword) U_CHAR *buf, *limit;
FILE_BUF *op;
struct directive *keyword;
{
  if (pedantic)
...
static int do_if(buf, limit, op, keyword) U_CHAR *buf, *limit;
FILE_BUF *op;
struct directive *keyword;
{
  HOST_WIDE_INT value;
...
static int do_elif(buf, limit, op, keyword) U_CHAR *buf, *limit;
FILE_BUF *op;
struct directive *keyword;
{
  HOST_WIDE_INT value;
...
static HOST_WIDE_INT eval_if_expression(buf, length) U_CHAR *buf;
int length;
{
  FILE_BUF temp_obuf;
  ...
  save_defined = install((U_CHAR *)"defined", -1, T_SPEC_DEFINED, NULL_PTR, -1);
  pcp_inside_if = 1;
...
static int do_xifdef(buf, limit, op, keyword) U_CHAR *buf, *limit;
FILE_BUF *op;
struct directive *keyword;
{
  int skip;
  ...
  if (ip->fname != 0 && keyword->type == T_IFNDEF) {
    ...
    while (p != directive_start) {
      ...
      else if (c == '/' && (*p == '*' || (cplusplus_comments && *p == '/'))) {
        /* Skip this comment.  */
        int junk = 0;
...
static void conditional_skip(ip, skip, type, control_macro, op) FILE_BUF *ip;
int skip;
enum node_type type;
U_CHAR *control_macro;
FILE_BUF *op;
...
static void skip_if_group(ip, any, op) FILE_BUF *ip;
int any;
FILE_BUF *op;
{
  ...
  while (bp < endb) {
    ...
    case '\"':
      ...
      /* Fall through.  */
    case '\'':
      bp = skip_quoted_string(bp - 1, endb, ip->lineno, &ip->lineno, NULL_PTR,
                              NULL_PTR);
      break;
    case '\\':
    ...
    sharp_token:
      ...
      for (kt = directive_table; kt->length >= 0; kt++) {
        IF_STACK_FRAME *temp;
        if (ident_length == kt->length && bcmp(cp, kt->name, kt->length) == 0) {
  ...
  if (output_conditionals && op != 0) {
    ...
    bcopy((char *)beg_of_group, (char *)op->bufp, beg_of_line - beg_of_group);
...
static int do_else(buf, limit, op, keyword) U_CHAR *buf, *limit;
FILE_BUF *op;
struct directive *keyword;
{
  FILE_BUF *ip = &instack[indepth];
...
static int do_endif(buf, limit, op, keyword) U_CHAR *buf, *limit;
FILE_BUF *op;
struct directive *keyword;
{
  if (pedantic) {
  ...
  if (if_stack == instack[indepth].if_stack)
    ...
    if (temp->control_macro != 0) {
      ...
      /* If we get here, this #endif ends a #ifndef
         ...
         file in a -imacros.  */
      if (indepth != 0 && !(indepth == 1 && no_record_file) &&
...
static void validate_else(p, limit) register U_CHAR *p;
register U_CHAR *limit;
{
...
static U_CHAR *skip_to_end_of_comment(ip, line_counter,
                                      nowarn) register FILE_BUF *ip;
...
static U_CHAR *skip_quoted_string(bp, limit, start_line, count_newlines,
                                  backslash_newlines_p,
                                  eofp) register U_CHAR *bp;
register U_CHAR *limit;
int start_line;
int *count_newlines;
...
static char *quote_string(dst, src) char *dst, *src;
{
  U_CHAR c;
...
static U_CHAR *skip_paren_group(ip) register FILE_BUF *ip;
{
...
static void output_line_directive(ip, op, conditional, file_change) FILE_BUF *ip
    ,
    *op;
int conditional;
enum file_change_code file_change;
{
  ...
  line_directive_buf = (char *)malloc(4 * strlen(ip->nominal_fname) + 100);
...
static void macroexpand(hp, op) HASHNODE *hp;
FILE_BUF *op;
{
  int nargs;
        ...
        if (ap->stringify)
          ...
          if (args[ap->argno].expanded == 0) {
            ...
            obuf = expand_to_temp_buffer(
                args[ap->argno].raw,
                args[ap->argno].raw + args[ap->argno].raw_length, 1, 0);
...
static char *macarg(argptr, rest_args) register struct argdata *argptr;
int rest_args;
{
  FILE_BUF *ip = &instack[indepth];
  int paren = 0;
  ...
  /* Try to parse as much of the argument as exists at this
     input stack level.  */
  U_CHAR *bp = macarg1(ip->bufp, ip->buf + ip->length, ip->macro, &paren,
  ...
  if (!(ip->fname != 0 && (newlines != 0 || comments != 0)) &&
    ...
    while (bp == ip->buf + ip->length) {
      ...
      newlines = 0;
      comments = 0;
      bp = macarg1(ip->bufp, ip->buf + ip->length, ip->macro, &paren, &newlines,
                   &comments, rest_args);
      final_start = bufsize;
      bufsize += bp - ip->bufp;
      extra += newlines;
...
static U_CHAR *macarg1(start, limit, macro, depthptr, newlines, comments,
                       rest_args) U_CHAR *start;
register U_CHAR *limit;
struct hashnode *macro;
...
static int discard_comments(start, length, newlines) U_CHAR *start;
int length;
int newlines;
{
  register U_CHAR *ibp;
...
static int change_newlines(start, length) U_CHAR *start;
int length;
{
  register U_CHAR *ibp;
  register U_CHAR *obp;
...
char *my_strerror(errnum) int errnum;
{
  char *result;
...
static void verror(msg, args) char *msg;
va_list args;
{
  int i;
  FILE_BUF *ip = NULL;
...
static void error_from_errno(name) char *name;
{
  int e = errno;
  int i;
  FILE_BUF *ip = NULL;
...
static void vwarning(msg, args) char *msg;
va_list args;
{
  int i;
  FILE_BUF *ip = NULL;
...
#else
    error_with_line(line, PRINTF_ALIST(msg)) int line;
PRINTF_DCL(msg)
#endif
{
  va_list args;
...
static void verror_with_line(line, msg, args) int line;
char *msg;
va_list args;
{
  int i;
...
#else
    warning_with_line(line, PRINTF_ALIST(msg)) int line;
PRINTF_DCL(msg)
#endif
{
  va_list args;
...
static void vwarning_with_line(line, msg, args) int line;
char *msg;
va_list args;
{
  int i;
...
#else
    pedwarn_with_line(line, PRINTF_ALIST(msg)) int line;
PRINTF_DCL(msg)
#endif
{
  va_list args;
...
#else
    pedwarn_with_file_and_line(file, line, PRINTF_ALIST(msg)) char *file;
int line;
PRINTF_DCL(msg)
#endif
{
...
static int line_for_error(line) int line;
{
  int i;
  int line1 = line;
...
static int grow_outbuf(obuf, needed) register FILE_BUF *obuf;
register int needed;
{
  register U_CHAR *p;
  int minsize;
...
static HASHNODE *install(name, len, type, value, hash) U_CHAR *name;
int len;
enum node_type type;
char *value;
int hash;
...
HASHNODE *lookup(name, len, hash) U_CHAR *name;
int len;
int hash;
{
  register U_CHAR *bp;
...
static void delete_macro(hp) HASHNODE *hp;
{
...
static int hashf(name, len, hashsize) register U_CHAR *name;
register int len;
int hashsize;
{
  register int r = 0;
...
static void dump_single_macro(hp, of) register HASHNODE *hp;
FILE *of;
{
...
static void dump_defn_1(base, start, length, of) U_CHAR *base;
int start;
int length;
FILE *of;
{
  ...
  if (traditional)
    ...
    while (p < limit) {
      if (*p == '\"' || *p == '\'') {
        U_CHAR *p1 =
            skip_quoted_string(p, limit, 0, NULL_PTR, NULL_PTR, NULL_PTR);
...
static void dump_arg_n(defn, argnum, of) DEFINITION *defn;
int argnum;
FILE *of;
{
...
static void initialize_builtins(inp, outp) FILE_BUF *inp;
FILE_BUF *outp;
{
  ...
  install((U_CHAR *)"__USER_LABEL_PREFIX__", -1, T_USER_LABEL_PREFIX_TYPE,
          NULL_PTR, -1);
  install((U_CHAR *)"__REGISTER_PREFIX__", -1, T_REGISTER_PREFIX_TYPE, NULL_PTR,
          -1);
  ...
  if (debug_output) {
    ...
    pass_thru_directive(udirective, &udirective[strlen(directive)], outp, dp);
    ...
    sprintf(directive, " __DATE__ \"%s %2d %4d\"\n",
            monthnames[timebuf->tm_mon], timebuf->tm_mday,
...
static void make_definition(str, op) char *str;
FILE_BUF *op;
{
  FILE_BUF *ip;
  struct directive *kt;
  ...
  if (*p == 0) {
    ...
    buf = (U_CHAR *)malloc(2 * strlen(str) + 1);
    ...
    while (*p) {
      if (*p == '\"' || *p == '\'') {
        int unterminated = 0;
        U_CHAR *p1 = skip_quoted_string(p, p + strlen((char *)p), 0, NULL_PTR,
                                        NULL_PTR, &unterminated);
        if (unterminated)
          return;
        while (p != p1)
          *q++ = *p++;
...
static void make_undef(str, op) char *str;
FILE_BUF *op;
{
  FILE_BUF *ip;
  struct directive *kt;
...
static void make_assertion(option, str) char *option;
char *str;
{
  FILE_BUF *ip;
  struct directive *kt;
  ...
  /* Copy the entire option so we can modify it.  */
  buf = (U_CHAR *)malloc(strlen(str) + 1);
  strcpy((char *)buf, str);
  /* Scan for any backslash-newline and remove it.  */
  p = q = buf;
  while (*p) {
...
static struct file_name_list *
    new_include_prefix(prev_file_name, component, prefix,
                       name) struct file_name_list *prev_file_name;
char *component;
char *prefix;
char *name;
{
  ...
  {
    struct file_name_list *dir = ((struct file_name_list *)xmalloc(
        sizeof(struct file_name_list) + strlen(prefix) + strlen(name) + 2));
    size_t len;
    ...
    if (prev_file_name && INO_T_EQ(prev_file_name->st.st_ino, dir->st.st_ino) &&
...
static void append_include_chain(first, last) struct file_name_list *first,
    *last;
{
  struct file_name_list *dir;
...
static int quote_string_for_make(dst, src) char *dst;
char *src;
{
  char *p = src;
  int i = 0;
...
static void deps_output(string, spacer) char *string;
int spacer;
{
...
static void fatal(PRINTF_ALIST(msg)) PRINTF_DCL(msg) {
  va_list args;
...
static void perror_with_name(name) char *name;
{
  fprintf(stderr, "%s: %s: %s\n", progname, name, my_strerror(errno));
  errors++;
}

static void pfatal_with_name(name) char *name;
{
...
static void pipe_closed(signo)
    /* If this is missing, some compilers complain.  */
    int signo;
{ fatal("output pipe has been closed"); }

static void memory_full() { fatal("Memory exhausted."); }
...
static GENERIC_PTR xrealloc(old, size) GENERIC_PTR old;
size_t size;
{
...
static GENERIC_PTR xcalloc(number, size) size_t number, size;
{
...
static char *savestring(input) char *input;
{
...
static void hack_vms_include_specification(fname, vaxc_include) char *fname;
int vaxc_include;
{
  ...
  if (((cp - fname) > 1) && ((cp[-1] == ']') || (cp[-1] == '>'))) {
    if (cp[-2] != '.') {
       ...
       * generation of the 000000 root directory spec (which does not belong
       * here in this case).
       */
      cp -= 2; /* Strip ".]" */
      cp1--;
     ...
     * searching the users default directory (as if they did a #include
     * "asdf.h").
     *
     ...
     */
    if (cp[-1] != ':')
      *cp2++ = ':'; /* dev not in spec.  take first dir */
...
static FILE *VMS_freopen(fname, type, oldfile) char *fname;
char *type;
FILE *oldfile;
{
#undef freopen /* Get back the real freopen routine.  */
  if (strcmp(type, "w") == 0)
    return freopen(fname, type, oldfile, "mbc=16", "deq=64", "fop=tef",
                   "shr=nil");
  return freopen(fname, type, oldfile, "mbc=16");
}

static FILE *VMS_fopen(fname, type) char *fname;
char *type;
{
#undef fopen /* Get back the real fopen routine.  */
  ...
  if (*type == 'w')
    return (*vmslib_fopen)(fname, type, "mbc=32", "deq=64", "fop=tef",
                           "shr=nil");
  else
    return (*vmslib_fopen)(fname, type, "mbc=32");
}

static int VMS_open(fname, flags, prot) char *fname;
int flags;
int prot;
{
...
static int VMS_fstat(fd, statbuf) int fd;
struct stat *statbuf;
{
...
static int VMS_stat(name, statbuf) const char *name;
struct stat *statbuf;
{
